        ...
        <>
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl bg-zinc-950 p-4">
                <div>
                    ...
                    <p className="text-sm text-zinc-500">{fasilitas.length} facilities</p>
                </div>
                ...
                <form onSubmit={handleCreate} className="flex flex-wrap items-end gap-3 rounded-xl border border-zinc-800 bg-zinc-900 p-4 shadow-sm shadow-black/20">
                    <div className="flex-1">
                        <label className="mb-1 block text-xs font-medium text-zinc-200">Nama Fasilitas</label>
                        <input value={nama} onChange={(e) => setNama(e.target.value)} required className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder-zinc-500 focus:border-teal-500 focus:outline-none" placeholder="WiFi, Parkir..." />
                    </div>
                ...
                <div className="overflow-x-auto rounded-xl border border-zinc-800">
                    <table className="w-full text-left text-sm">
                        ...
                        <tbody className="divide-y divide-zinc-800">
                            {fasilitas.map((f) => (
                                <tr key={f.id} className="bg-zinc-950 hover:bg-zinc-900/50">
                                    {editingId === f.id ? (
                                        <>
                                            ...
                                            <td className="px-4 py-2 text-zinc-500">{f.ikon || '—'}</td>
                                            <td className="px-4 py-2 text-zinc-400">{f.wisatas_count}</td>
                                            <td className="flex gap-2 px-4 py-2">
                                                <button onClick={() => handleUpdate(f.id)} className="rounded bg-teal-600 px-2 py-1 text-xs font-medium text-white hover:bg-teal-700">Save</button>
                                        ...
                                        <>
                                            <td className="px-4 py-3 font-medium text-zinc-100">{f.nama_fasilitas}</td>
                                            <td className="px-4 py-3 text-zinc-500">{f.ikon || '—'}</td>
                                            <td className="px-4 py-3">
                                                <span className="rounded-full bg-teal-900/50 px-2.5 py-0.5 text-xs font-medium text-teal-400">{f.wisatas_count}</span>
                                            </td>
                                            <td className="flex gap-2 px-4 py-3">
                                                <button onClick={() => { setEditingId(f.id); setEditNama(f.nama_fasilitas); }} className="rounded-md p-1.5 text-zinc-500 hover:bg-zinc-800 hover:text-teal-400">
                                                ...
                                                <button onClick={() => handleDelete(f.id, f.nama_fasilitas)} className="rounded-md p-1.5 text-zinc-500 hover:bg-red-900/30 hover:text-red-400">
